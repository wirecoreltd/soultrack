...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      setRoles(Array.isArray(parsedRoles) ? parsedRoles.map(r => r.trim()) : [parsedRoles.trim()]);
    } catch {
  ...
  return (
    <div className="relative min-h-screen flex flex-col items-center justify-center p-6 text-center" style={{ background: "linear-gradient(135deg, #2E3192 0%, #92EFFD 100%)" }}>
      ...
      <div className="text-white text-lg font-handwriting-light max-w-2xl">
        Car le corps ne se compose pas d’un seul membre, mais de plusieurs. <br />1 Corinthiens 12:14 ❤️
      </div>
    </div>
  );
}
