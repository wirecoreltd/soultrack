...
function RapportBaptemes() {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center px-4 py-6 bg-[#333699]">
      ...
      {/* FORM */}
      <form onSubmit={handleSubmit} className="bg-white/10 p-4 rounded-xl mt-4 w-full max-w-lg">
        <input
          type="date"
          value={formData.date}
          onChange={(e) =>
        ...
        <button className="w-full mt-3 bg-blue-500 text-white p-2 rounded">
              ...
              {expandedMonths[key] &&
                grouped[key].map((r) => (
                  <div key={r.id} className="text-white p-2">
                    {formatDateFR(r.date)} - {r.baptise_par} - {r.hommes + r.femmes}
                  </div>
                ))}
            </div>
          );
