...
export default function Evangelisation() {
  ...
  const sendContacts = async () => {
    ...
    try {
      ...
      // Mettre les doublons dans l’état pour affichage
      ...
      {doublons.length > 0 && (
        <div className="bg-blue-100/30 border-l-4 border-blue-500/70 p-4 mb-4 w-full max-w-6xl rounded shadow">
          ...
          <p className="text-sm text-blue-700 mb-2">
            Ces contacts sont déjà enregistrés dans les suivis. Vous pouvez les garder sur la page ou les retirer temporairement.
          </p>
          {doublons.map((c) => (
