...
export default function LinkEglise() {
  ...
  const handleSelectInvitation = (inv, action) => {
    ...
    // 🔹 Scroll formulaire en haut
    setTimeout(() => formRef.current?.scrollIntoView({ behavior: "smooth" }), 100);
  };

  // 🔹 Action sur invitation
  const handleAction = async () => {
    ...
    try {
      ...
      if (modeAction === "supprimer") {
        // 🔹 Supprimer → DELETE + branches
        await supabase.from("eglise_supervisions").delete().eq("id", selectedInvitation.id);
        if (selectedInvitation.supervisee_branche_id) {
          await supabase.from("branches").update({ superviseur_branche_id: null, superviseur_nom: null }).eq("id", selectedInvitation.supervisee_branche_id);
        }
      }

      if (modeAction === "casser") {
        // 🔹 Casser le lien → statut lien_casse + branches
        await supabase.from("eglise_supervisions").update({ statut: "lien_casse" }).eq("id", selectedInvitation.id);
        if (selectedInvitation.supervisee_branche_id) {
          await supabase.from("branches").update({ superviseur_branche_id: null, superviseur_nom: null }).eq("id", selectedInvitation.supervisee_branche_id);
        }
      }
      ...
      if (!selectedInvitation && modeAction === null) {
        ...
        if (canal === "whatsapp") window.open(`https://wa.me/?text=Nouvelle invitation`, "_blank");
      }
    ...
    <div className="min-h-screen bg-[#333699] text-white p-4 flex flex-col items-center">
      ...
      <div ref={formRef} className="w-full max-w-md rounded-2xl shadow-lg p-6 space-y-4 mb-10 bg-white/10">
        ...
        <div>
          ...
          />
          {selectedInvitation && modeAction === "supprimer" && (
        ...
        <select className="w-full border rounded-xl px-3 py-2 text-black"
        ...
        <button onClick={handleAction}
          disabled={modeAction !== "supprimer" && (!eglise.nom || !eglise.branche || !eglise.pays)}
          className={`w-full py-2 rounded-xl text-white font-semibold ${
            modeAction !== "supprimer" && (!eglise.nom || !eglise.branche || !eglise.pays)
      ...
      <div className="w-full max-w-5xl overflow-x-auto">
        ...
        {invitations.map((inv) => {
          ...
          if (inv.statut.toLowerCase() === "supprimer") return null; // Supprimer n'apparaît plus
          return (
