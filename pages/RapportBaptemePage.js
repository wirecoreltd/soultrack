...
function RapportBaptemes() {
  ...
  const groupByMonth = (items) => {
    ...
    // Tri décroissant par clé (YYYY-MM)
    ...
    <div className="min-h-screen flex flex-col items-center px-4 sm:px-6 py-6 bg-[#333699]">
      ...
      <div className="max-w-3xl w-full mb-6 text-center">
        <p className="italic text-base text-white/90">
          ...
          <span className="text-blue-300 font-semibold">nouveaux baptisés</span>.
      ...
      {showTable && (
        <>
          {/* --- TOTAL GLOBAL + BOUTON TOUT OUVRIR/FERMER --- */}
          <div className="w-full max-w-4xl mx-auto mt-6 flex flex-col sm:flex-row justify-between items-center gap-3 px-2">
            <div className="text-white text-sm font-semibold bg-white/10 px-4 py-2 rounded-xl">
              Total global :
              <span className="text-orange-400 ml-2">
                {totalGlobal.hommes}H / {totalGlobal.femmes}F = {totalGlobal.hommes + totalGlobal.femmes} baptisés
              </span>
            </div>
            <button
              onClick={() => {
                const allKeys = groupedMonths.map(g => g.key);
                const allOpen = allKeys.every(k => expandedMonths[k]);
                const newState = {};
            ...
            >
              {groupedMonths.every(g => expandedMonths[g.key]) ? "Tout réduire ▲" : "Tout ouvrir ▼"}
            </button>
          </div>

          {/* --- TABLEAU DESKTOP --- */}
          <div className="hidden md:flex w-full max-w-4xl overflow-x-auto mt-3 justify-center">
            <div className="w-full space-y-3">
              {groupedMonths.map(group => {
                ...
                return (
                  <div key={group.key} className="rounded-2xl overflow-hidden border border-white/20">
                    {/* EN-TÊTE DU MOIS (cliquable) */}
                    <button
                      onClick={() => toggleMonth(group.key)}
                      className="w-full flex items-center justify-between px-5 py-3 bg-white/15 hover:bg-white/25 transition text-white font-bold text-left"
                    >
                      <span className="flex items-center gap-2 text-emerald-300 text-base">
                        <span>{isOpen ? "▼" : "▶"}</span>
                        {group.label}
                      </span>
                      <span className="text-sm text-orange-300 font-semibold">
                        {monthTotal.hommes}H / {monthTotal.femmes}F —{" "}
                        <span className="text-white">{monthTotal.hommes + monthTotal.femmes} baptisés</span>
                      </span>
                    </button>
                    ...
                    {isOpen && (
                      <div className="w-full">
                        {/* En-têtes colonnes */}
                        <div className="flex text-xs font-semibold uppercase text-white/60 px-5 py-2 bg-white/5 whitespace-nowrap border-b border-white/10">
                          <div className="min-w-[180px]">Date</div>
                          ...
                          <div className="min-w-[130px] text-center">Actions</div>
                        </div>
                        ...
                        {group.items.map((r) => {
                          ...
                          return (
                            <div
                              ...
                              className="flex items-center px-5 py-3 bg-white/10 hover:bg-white/20 transition border-l-4 border-blue-500 border-b border-b-white/5"
                            >
                              ...
                              <div className="min-w-[100px] text-center text-white font-bold">{total}</div>
                        ...
                        {/* SOUS-TOTAL DU MOIS */}
                        <div className="flex items-center px-5 py-2 bg-white/5 border-t border-white/20">
                          <div className="min-w-[180px] text-white/60 text-xs uppercase">Sous-total</div>
                          <div className="min-w-[200px]"></div>
                          <div className="min-w-[100px] text-center text-amber-300 font-semibold">{monthTotal.hommes}</div>
                          <div className="min-w-[100px] text-center text-amber-300 font-semibold">{monthTotal.femmes}</div>
                          <div className="min-w-[100px] text-center text-amber-300 font-semibold">{monthTotal.hommes + monthTotal.femmes}</div>
              ...
              {/* TOTAL GLOBAL */}
              <div className="flex items-center px-5 py-3 mt-1 border border-white/30 bg-white/10 rounded-xl">
                <div className="min-w-[180px] text-white font-bold">TOTAL GLOBAL</div>
                <div className="min-w-[200px]"></div>
                <div className="min-w-[100px] text-center text-orange-400 font-semibold">{totalGlobal.hommes}</div>
                <div className="min-w-[100px] text-center text-orange-400 font-semibold">{totalGlobal.femmes}</div>
                <div className="min-w-[100px] text-center text-orange-400 font-semibold">{totalGlobal.hommes + totalGlobal.femmes}</div>
                <div className="min-w-[130px]"></div>
              </div>
            </div>
          </div>
          ...
          <div className="md:hidden w-full mt-4 flex flex-col gap-3">
            {groupedMonths.map(group => {
              ...
              return (
                <div key={group.key} className="rounded-2xl overflow-hidden border border-white/20">
                  ...
                  <button
                    onClick={() => toggleMonth(group.key)}
                    className="w-full flex items-center justify-between px-4 py-3 bg-white/15 hover:bg-white/25 transition text-white font-bold"
                  >
                    ...
                    <span className="text-xs text-orange-300">
                      {monthTotal.hommes + monthTotal.femmes} baptisés
                    </span>
                  </button>
                    ...
                    <div className="flex flex-col gap-2 p-2 bg-white/5">
                      {group.items.map((r) => {
                        ...
                        return (
                          <div
                            key={r.id + r.baptise_par}
                            className="bg-white/10 text-white rounded-xl px-4 py-3 flex flex-col gap-2 shadow border-l-4 border-blue-500"
                          >
                            <div className="text-amber-300 text-right opacity-80 text-sm">
                              {formatDateFR(r.date)}
                            </div>
                            ...
                            <button
                              onClick={() => handleEdit(r)}
                              className="mx-auto text-amber-300 mt-1 hover:scale-110 transition text-sm"
                            >
                              ✏️ Modifier
                            </button>
                          </div>
                      ...
                      {/* SOUS-TOTAL MOBILE */}
                      <div className="text-center text-amber-300 font-semibold text-sm py-2 border-t border-white/20">
                        Sous-total {group.label} : {monthTotal.hommes}H / {monthTotal.femmes}F = {monthTotal.hommes + monthTotal.femmes}
                      </div>
                    </div>
                  )}
                </div>
            ...
            {/* TOTAL GLOBAL MOBILE */}
            <div className="bg-white/10 rounded-xl px-4 py-3 text-center text-orange-400 font-bold text-sm border border-white/20">
              TOTAL GLOBAL : {totalGlobal.hommes}H / {totalGlobal.femmes}F = {totalGlobal.hommes + totalGlobal.femmes}
            </div>
          </div>
        </>
      )}
