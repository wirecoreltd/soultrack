...
const translations = {
  fr: {
    ...
    voirServiteurs: "👥 Voir les serviteurs",
    cacherServiteurs: "▲ Masquer",
    aucunNom: "Aucun nom disponible",
  },
  en: {
    title: "Report",
    ...
    voirServiteurs: "👥 View servants",
    cacherServiteurs: "▲ Hide",
    aucunNom: "No names available",
  },
};
...
// ─── CARTE MINISTÈRE (avec détails serviteurs) ─────────────────
function CarteMinistere({ ministere, total, idx, serviteursCount, nomsServiteurs, t }) {
  const [open, setOpen] = useState(false);
  const cfg = getConfig(ministere);
  const pct = serviteursCount > 0 ? Math.round((total / serviteursCount) * 100) : 0;
  const noms = nomsServiteurs[ministere] || [];
  ...
  return (
    <div className="bg-white/10 rounded-2xl overflow-hidden">
      ...
      <div className="flex items-center gap-2 px-4 pb-3">
        ...
        <BarreProgression
          pct={(total / Math.max(serviteursCount, 1)) * 100 * (100 / Math.max(pct, 1))}
          color={cfg.bar}
        />
      ...
      {open && (
        <div className="border-t border-white/10 px-4 pb-4 pt-3 flex flex-col gap-2">
          {noms.length === 0 ? (
            <p className="text-white/30 text-xs text-center py-2">{t.aucunNom}</p>
          ) : (
            <div className="flex flex-wrap gap-2">
              {noms.map((nom, i) => (
                <span
                  key={i}
                  className="bg-white/10 rounded-full px-3 py-1 text-xs text-white/80 font-medium"
                >
                  {nom}
                </span>
              ))}
...
function RapportMinistere() {
  ...
  const [rapports, setRapports] = useState({ lignes: [], serviteursCount: 0, hommes: 0, femmes: 0 });
  const [nomsServiteurs, setNomsServiteurs] = useState({});
  ...
  const fetchRapport = async (overrideModePerso = null) => {
    ...
    try {
      ...
      const { data: membresData } = await supabase
        ...
        .select("id, etat_contact, star, sexe, nom, prenom")
      ...
      // Set des IDs star pour filtrer les stats
      ...
      (membresData || []).forEach(m => {
        nomMap[m.id] = `${m.prenom || ""} ${m.nom || ""}`.trim() || m.id;
      });
      ...
      const seen = new Set();
      const counts = {};
      // Map ministere → Set de membre_ids
      const ministereMembreIds = {};
      ...
      (statsData || []).forEach(s => {
        ...
        ministeres.forEach(ministere => {
          ...
          if (!ministereMembreIds[ministere]) ministereMembreIds[ministere] = new Set();
          ministereMembreIds[ministere].add(s.membre_id);
        });
      });
      ...
      // Construire les noms par ministère
      const noms = {};
      Object.entries(ministereMembreIds).forEach(([ministere, ids]) => {
        noms[ministere] = Array.from(ids).map(id => nomMap[id]).filter(Boolean);
      });

      setNomsServiteurs(noms);
      ...
      <div className="w-full max-w-3xl flex flex-col gap-6">
        ...
        {loading ? (
          ...
          <div className="flex flex-col gap-3">
            ...
            {/* Cartes ministères (top 10) avec détails au clic */}
            {rapports.lignes.slice(0, 10).map(({ ministere, total }, idx) => (
              <CarteMinistere
                ...
                total={total}
                idx={idx}
                serviteursCount={rapports.serviteursCount}
                nomsServiteurs={nomsServiteurs}
                t={t}
              />
            ))}
