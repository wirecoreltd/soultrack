...
export default function AdminAmeliorationsPage() {
  return (
    ...
    >
      ...
      <div className="text-center mb-6">
        <h1 className="text-2xl font-bold mt-4 mb-6 text-white">
          Gestion des <span className="text-emerald-300">Améliorations</span>
        </h1>
        <p className="italic text-base text-white/90 max-w-2xl">
          Consultez et traitez les suggestions d'amélioration soumises par les utilisateurs.
        </p>
      </div>
      ...
      <div className="w-full max-w-6xl grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 justify-items-center">
        {loading ? (
          ...
          <p className="text-white col-span-full">Aucune amélioration trouvée.</p>
        ) : (
          filtered.map((t) => (
            <div
              key={t.id}
