    ...
    <div className="min-h-screen w-full flex flex-col items-center p-6" style={{ background: "linear-gradient(135deg, #2E3192 0%, #92EFFD 100%)" }}>
      ...
      {contacts === null ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 w-full max-w-5xl">
          ...
          {contacts.map((m) => (
            <div key={m.id} className="bg-white/10 rounded-2xl p-4 border-l-4 hover:bg-white/20 transition" style={{ borderLeftColor: getBorderColor(m) }}>
              <div className="text-white font-bold">{m.prenom} {m.nom}</div>
              <div className="text-white">📱 {m.telephone || "—"}</div>
              <div className="text-white">🏙️ {m.ville || "—"}</div>
            </div>
          ))}
        </div>
      ) : (
