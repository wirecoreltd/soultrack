...
function StatGlobalPage() {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      if (data?.branche_id) {
        ...
        // Récupérer toutes les branches enfants de cette branche
        const fetchChildBranches = async (parentId) => {
          const { data: enfants } = await supabase
            .from("eglises")
            .select("id")
            .eq("parent_eglise_id", parentId);

          let ids = [parentId];
          if (enfants?.length) {
            ids = ids.concat(enfants.map(e => e.id));
          }
          return ids;
        };

        const allBranchIds = await fetchChildBranches(data.branche_id);
  ...
  const fetchStats = async () => {
    if (!userBrancheId) {
      alert("Profil utilisateur en cours de chargement…");
      return;
    }

    if (!branchIds.length) {
      alert("Aucune branche trouvée pour l'utilisateur.");
      return;
    }
    ...
    try {
      ...
      console.error("Erreur fetchStats:", error.message);
    }
  ...
  return (
    <div className="min-h-screen flex flex-col items-center p-6 bg-[#333699]">
      ...
      <div className="bg-white/10 p-6 rounded-2xl shadow-lg mt-6 flex gap-4 flex-wrap text-white">
        ...
        <button
          onClick={fetchStats}
          disabled={!userBrancheId || loading}
          className={`bg-[#2a2f85] px-6 py-2 rounded-xl hover:bg-[#1f2366] ${(!userBrancheId || loading) ? "opacity-50 cursor-not-allowed" : ""}`}
        >
