// pages/famille/ajouter-membre-famille.js
...
const translations = {
  fr: {
    pageTitle: "Ajouter un membre à ma",
...
function AjouterMembreFamilleContent() {
  ...
  // ✅ Attendre que router soit prêt avant de lire query
  const [routerReady, setRouterReady] = useState(false);
  useEffect(() => {
    if (router.isReady) setRouterReady(true);
  }, [router.isReady]);

  const urlEgliseId  = routerReady ? (router.query.eglise_id  || null) : null;
  const urlFamilleId = routerReady ? (router.query.famille_id || null) : null;
  const isFromLink   = !!urlEgliseId && !!urlFamilleId;

  const [familleInfo, setFamilleInfo] = useState(null);
  // ✅ Lire famille_full uniquement quand router est prêt
  useEffect(() => {
    if (router.isReady && router.query.famille_full) {
      setFamilleInfo(decodeURIComponent(router.query.famille_full));
    }
  }, [router.isReady, router.query.famille_full]);
  ...
  // ✅ Attendre routerReady avant de résoudre le scope
  useEffect(() => {
  ...
  // ✅ Fetch familles uniquement si connecté et routerReady
  useEffect(() => {
    if (!routerReady) return;
    if (!userScope.eglise_id) return;
    ...
    const fetchFamilles = async () => {
      ...
      if (!userId) { alert(t.errUser); return; }

      const { data, error } = await supabase
        .from("familles").select("id, ville, famille")
        .eq("responsable_id", userId).eq("eglise_id", userScope.eglise_id);

      if (error || !data || data.length === 0) { alert(t.errFamille); return; }
      setFamilles(data);
      if (data.length === 1) setFormData(prev => ({ ...prev, famille_id: data[0].id }));
    };
    fetchFamilles();
  }, [userScope, isFromLink, routerReady]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!noPhone && !formData.telephone) { alert(t.errNoPhone); return; }
    if (!familleIdFinal) { alert(t.errFamilleSelect); return; }
    ...
    try {
      ...
      setFormData({
        ...
        date_venu: new Date().toISOString().slice(0, 10),
        besoin: [], autreBesoin: "", besoinLibre: "",
  ...
  const handleCancel = () => {
    setFormData({
      nom: "", prenom: "", sexe: "",
      telephone: phonePrefix || "", ville: "", age: "",
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-purple-200 via-pink-100 to-yellow-100 p-6">
      <div className="w-full max-w-md bg-white p-8 rounded-3xl shadow-lg relative">
        ...
        <div className="flex flex-col items-center mb-3 sm:mb-6 gap-2">
          ...
          {/* ✅ Nom de la famille affiché depuis l'URL */}
          {familleInfo && (
        ...
        <h1 className="text-2xl font-bold mt-4 mb-6 text-center text-black">
          {t.pageTitle} <span className="text-[#333699]">{t.pageTitleHighlight}</span>
        </h1>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          {!isFromLink && Familles.length > 1 && (
            <select name="famille_id" value={formData.famille_id}
              onChange={handleChange} className="input" required>
              <option value="">{t.chooseFamille}</option>
              {Familles.map(c => (
                <option key={c.id} value={c.id}>{c.ville} - {c.famille}</option>
              ))}
            </select>
          )}
          ...
          <textarea name="infos_supplementaires" placeholder="..."
