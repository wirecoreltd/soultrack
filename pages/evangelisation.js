        ...
        >
          ...
          {(selectedTargetType === "cellule" ? cellules : conseillers).map((c) => (
            <option key={c.id} value={c.id}>
              {selectedTargetType === "cellule" ? (c.ville ? `${c.cellule_full} - ${c.ville}` : c.cellule_full) : `${c.prenom} ${c.nom}`}
            </option>
          ))}
        </select>
      )}
    ...
    <div className="w-full max-w-6xl flex flex-col items-center">
      ...
      {doublons.length > 0 && (
        <div className="bg-blue-100/30 border-l-4 border-blue-500/70 p-4 mb-4 w-full max-w-6xl rounded shadow">
          <p className="font-bold text-blue-800 mb-2">⚠️ {doublons.length === 1 ? "Contact déjà en suivi !" : "Contacts déjà en suivi !"}</p>
                  ...
                  >
                    ...
                    {detailsOpen[member.id] && (
                      <div className="text-sm mt-3 space-y-1">
                        ...
                        {/* Bouton supprimer si le contact est en doublon */}
