...
function RapportBaptemes() {
  ...
  /* CALCUL AUTOMATIQUE HOMMES/FEMMES */
  useEffect(()=>{
  ...
  return (
    <div className="min-h-screen flex flex-col items-center px-4 py-6 sm:p-6 bg-[#333699]">
      ...
      <div className="max-w-6xl w-full grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        {/* CANDIDATS */}
        <div className="bg-white/10 p-3 rounded-3xl shadow-lg text-white flex flex-col gap-2">
          <div className="bg-blue-900/40 border border-blue-300/30 text-white text-sm p-4 rounded-2xl">
            ℹ️ Cette liste contient les personnes qui <strong>n'ont pas encore été baptisées</strong> et qui <strong>souhaitent prendre leur baptême</strong>.
            <div className="mt-2">
              <button onClick={()=>router.push("/list-members")} className="underline text-amber-300 hover:text-amber-200 text-sm">
          ...
          <div className="flex flex-col overflow-y-auto max-h-[250px] sm:max-h-[300px] space-y-1">
            {candidats.map(c=>(
              <div key={c.id} className="flex justify-between items-center w-full px-2 py-1 rounded hover:bg-white/20 cursor-pointer"
                   onClick={()=>{
                     setFormData(prev=>({
                       ...prev,
                       hommes: c.sexe==="Homme"?1:0,
                       femmes: c.sexe==="Femme"?1:0
                     }));
                     setSelectedCandidats([c.id]);
                   }}>
                <span>{c.prenom} {c.nom}</span>
                <input type="checkbox" checked={selectedCandidats.includes(c.id)} readOnly className="accent-[#25297e]"/>
              </div>
            ))}
          </div>

          <button onClick={()=>router.push("/AddContactbaptise")} className="text-white font-semibold px-4 py-2 rounded shadow text-sm mt-2 w-full">
            ➕ Ajouter un Baptisé
          </button>
        </div>
      </div>
      ...
      {showTable && (
        <div className="w-full overflow-x-auto mt-6">
          <div className="min-w-[700px] space-y-2">
            {/* HEADER */}
            <div className="hidden sm:flex text-sm font-semibold uppercase text-white px-4 py-3 border-b border-white/30 bg-white/5 rounded-t-xl whitespace-nowrap">
            ...
            {groupedReports.map(([monthKey,monthRapports])=>{
              ...
              return(
                <div key={monthKey} className="space-y-1">
                  {/* Collapse Header */}
                  <div className="flex items-center px-4 py-2 rounded-lg bg-white/20 cursor-pointer border-l-4 border-blue-500"
                  ...
                  {(isExpanded||monthRapports.length===1)&&monthRapports.map(r=>{
                    ...
                    return(
                      <div key={r.id} className="flex flex-col sm:flex-row items-start sm:items-center px-4 py-3 rounded-lg bg-white/10 hover:bg-white/20 transition border-l-4 border-blue-500">
                        <div className="sm:min-w-[200px]"><strong>Date:</strong> {formatDateFR(r.date)}</div>
                        <div className="sm:min-w-[200px] text-center"><strong>Baptisé par:</strong> {r.baptise_par}</div>
                        <div className="sm:min-w-[120px] text-center"><strong>Hommes:</strong> {r.hommes}</div>
                        <div className="sm:min-w-[120px] text-center"><strong>Femmes:</strong> {r.femmes}</div>
                        <div className="sm:min-w-[120px] text-center text-white font-bold"><strong>Total:</strong> {total}</div>
                        <div className="sm:min-w-[150px] text-center">
