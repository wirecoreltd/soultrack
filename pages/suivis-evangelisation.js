  ...
  return (
    <div className="min-h-screen flex flex-col items-center p-6" style={{ background: "linear-gradient(135deg, #2E3192 0%, #92EFFD 100%)" }}>
      ...
      {loading ? (
          ...
          {suivis.map(m => (
            ...
            >
              <div className="flex flex-col items-center">
                ...
                <button
                  onClick={() => setDetailsOpen(prev => prev === m.id ? null : m.id)}
        ...
        <div className="w-full max-w-6xl overflow-x-auto flex flex-col items-center">
          ...
          {/* POPUP TABLE */}
          {detailsOpen && (() => {
            const member = suivis.find(s => s.id === detailsOpen);
            if (!member) return null;
            return <SuiviDetailsEvanPopup member={member} onClose={() => setDetailsOpen(null)} />;
          })()}
        </div>
      )}
    </div>
