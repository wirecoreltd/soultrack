...
export default function SuivisEvangelisation() {
  ...
  const toggleDetails = (m) => setDetailsMember(prev => (prev?.id === m.id ? null : m));
      ...
      {loading ? (
          ...
          {suivis.map(m => (
            <div key={m.id} className="bg-white rounded-2xl shadow-lg w-full transition-all duration-300 hover:shadow-2xl p-4 border-l-4" style={{ borderLeftColor: getBorderColor(m) }}>
              <div className="flex flex-col items-center">
                ...
                <button onClick={() => toggleDetails(m)} className="text-orange-500 underline text-sm mt-1">{detailsMember?.id === m.id ? "Fermer détails" : "Détails"}</button>
              </div>
              {detailsMember?.id === m.id && <DetailsPopup m={m} />}
            </div>
          ))}
        </div>
      ) : (
        /* Vue Table adaptée de membres-cellule */
        <div className="w-full max-w-6xl overflow-x-auto flex justify-center">
          <table className="w-full text-sm text-left text-white border-separate border-spacing-0">
            ...
            <tbody>
              {suivis.length === 0 ? (
                <tr><td colSpan={5} className="px-4 py-2 text-white text-center">Aucun évangélisé</td></tr>
              ) : suivis.map(m => (
                <tr key={m.id} className="hover:bg-white/10 transition duration-150 border-b border-gray-300">
                  ...
                  <td className="px-4 py-2">
                    <div className="flex gap-3">
                      <button onClick={() => toggleDetails(m)} className="text-orange-500 underline text-sm">{detailsMember?.id === m.id ? "Fermer" : "Détails"}</button>
                    </div>
                  </td>
                </tr>
              ))}
