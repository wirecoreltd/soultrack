...
function RapportBaptemes() {
  ...
  const { eglise_id, branche_id } = formData;

  /* ---------------------------------- */
  /* FETCH USER + CANDIDATS */
  /* ---------------------------------- */
  useEffect(() => {
  ...
  const fetchCandidats = async (egliseId, brancheId) => {
  ...
  const fetchRapports = async () => {
    if (!eglise_id || !branche_id) return;
  ...
  const handleSubmit = async e => {
    ...
    fetchCandidats(eglise_id, branche_id);
    fetchRapports();
  };
  ...
  return (
    <div className="min-h-screen flex flex-col items-center p-6 bg-[#333699]">
      ...
      <div className="max-w-5xl w-full grid md:grid-cols-2 gap-6 mb-6">
        ...
        {/* Candidats */}
        <div className="bg-white/10 p-3 rounded-3xl shadow-lg text-white flex flex-col gap-4">
          <div className="bg-blue-900/40 border border-blue-300/30 text-white text-sm p-4 rounded-2xl text-center">
            ...
            <button onClick={()=>router.push("/list-members")} className="underline text-amber-300 hover:text-amber-200 text-sm">Voir la liste des membres</button>
          </div>

          {/* Sélection baptisés */}
          <div>
            <div className="flex justify-between items-center mb-2">
              <label className="font-semibold">Sélectionner les baptisés ({candidats.length})</label>
              <button onClick={()=>{
                if(selectedCandidats.length===0) setSelectedCandidats(candidats.map(c=>c.id));
                else setSelectedCandidats([]);
            ...
            <div className="flex flex-col overflow-y-auto max-h-[300px] space-y-1">
              {candidats.map(c=>(
                <div key={c.id} className="flex justify-between items-center w-full px-2 py-1 rounded hover:bg-white/20">
                  ...
                  <input type="checkbox" checked={selectedCandidats.includes(c.id)} onChange={()=>{
                    setSelectedCandidats(prev=>{
                      const updated = prev.includes(c.id) ? prev.filter(id=>id!==c.id) : [...prev,c.id];
