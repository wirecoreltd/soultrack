...
function StatGlobalPage() {
  ...
  const [branches, setBranches] = useState([]); // tableau des branches à afficher
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      if (profile?.branche_id) {
        ...
        // 🔹 récupérer la branche + ses enfants
        const { data: allBranches } = await supabase
          .from("branches")
          .select("id, nom")
          .or(`id.eq.${profile.branche_id},parent_id.eq.${profile.branche_id}`);
    ...
    const updatedBranches = await Promise.all(
      branches.map(async (branche) => {
        ...
        return {
          ...
          nom: branche.nom,
          rapports: [
            { label: "Culte", data: attendanceTotals },
            { label: "Evangelisation", data: evanTotals },
            { label: "Baptême", data: baptemeTotals },
            { label: "Formation", data: formationTotals },
            { label: "Serviteur", data: { hommes: hommesServ, femmes: femmesServ } },
            { label: "Cellules", data: { hommes: count } },
          ]
        };
      })
    );
  ...
  return (
    <div className="min-h-screen flex flex-col items-center p-6 bg-[#333699]">
      ...
      <h1 className="text-2xl font-bold mt-4 mb-6 text-center text-white">
        Rapport <span className="text-amber-300">Statistiques par Église</span>
      </h1>
      ...
      {!loading && branches.length > 0 && (
        <div className="w-full max-w-full mt-6 space-y-8">
          {affichageMois && <h2 className="text-xl font-semibold text-white mb-3">{affichageMois}</h2>}

          {branches.map((branche) => (
            <div key={branche.id} className="bg-white/10 rounded-2xl p-4">
              <h3 className="text-lg font-semibold text-white mb-3">{branche.nom}</h3>
