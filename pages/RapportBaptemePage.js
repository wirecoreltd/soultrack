...
function RapportBaptemes() {
  ...
  const handleSubmit = async(e)=>{
    ...
    fetchCandidats(egliseId,brancheId);
    fetchRapports();
  };
      ...
      <div className="max-w-6xl w-full space-y-6">
        ...
        <div className="bg-white/10 p-4 rounded-3xl text-white">
          <div className="flex justify-between mb-3">
            ...
            <button onClick={()=>setSelectedCandidats(selectedCandidats.length===0?candidats.map(c=>c.id):[])}
          ...
          <div className="max-h-[250px] overflow-y-auto space-y-1">
            {candidats.map(c=>(
              <div key={c.id} className="flex justify-between px-2 py-1 hover:bg-white/20 rounded">
                ...
                <input type="checkbox" checked={selectedCandidats.includes(c.id)}
                  onChange={()=>{
        ...
        <div className="bg-white/10 p-4 rounded-2xl flex gap-4 flex-wrap justify-center">
          ...
          <button onClick={fetchRapports} className="bg-indigo-600 px-6 py-2 rounded-xl text-white">Générer</button>
        </div>

        {/* TABLEAU DESKTOP */}
        {showTable && (
          <div className="hidden md:block w-full overflow-x-auto">
            {/* Garde ton tableau desktop exact comme avant */}
          </div>
        )}
            ...
            {rapports.map(r=>{
              ...
              return(
                <div key={r.id} className="bg-white/10 p-4 rounded-2xl text-white">
                  ...
                  <button onClick={()=>handleEdit(r)} className="text-amber-300 underline mt-2">Modifier</button>
                </div>
              );
            })}
          </div>
