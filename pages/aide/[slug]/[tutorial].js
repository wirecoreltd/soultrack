...
// ── Ne génère les pages QUE pour les slugs existants ────────────────────────
export async function getStaticPaths() {
  const slugs = new Set(categories.fr.map((c) => c.slug));
  const paths = Array.from(slugs).map((slug) => ({ params: { slug } }));
  return { paths, fallback: false };
}

// ── Ne renvoie que le sous-ensemble de données nécessaire à CETTE page ──────
// Le reste du fichier aideContent.js (toutes les autres catégories, tous les
// autres tutoriels, l'autre langue) reste côté build/serveur et n'est jamais
// envoyé au navigateur.
export async function getStaticProps({ params }) {
  const { slug } = params;

  const categoryFr = categories.fr.find((c) => c.slug === slug) || null;
  const categoryEn = categories.en.find((c) => c.slug === slug) || null;
  const contentFr = tutorials[slug]?.fr || null;
  const contentEn = tutorials[slug]?.en || null;

  return {
    props: { categoryFr, categoryEn, contentFr, contentEn },
  };
}

export default function AideCategoryPage({ categoryFr, categoryEn, contentFr, contentEn }) {
