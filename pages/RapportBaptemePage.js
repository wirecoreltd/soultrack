...
function RapportBaptemes() {
  ...
  /* FONCTION GENERER RAPPORT (fix bouton) */
  const fetchRapports = async () => {
    if (!formData.eglise_id || !formData.branche_id) {
      alert("Impossible de générer le rapport : église ou branche non définie.");
      return;
    }
    try {
      let query = supabase
  ...
  return (
    <div className="min-h-screen flex flex-col items-center p-6 bg-[#333699]">
      ...
      {/* 1️⃣ MESSAGE INFO */}
      <div className="bg-blue-900/40 border border-blue-300/30 text-white text-center text-sm p-4 rounded-2xl mb-6 max-w-2xl">
        ...
        <button
          ...
          className="underline text-amber-300 hover:text-amber-200 mt-2"
        >
          Voir la liste des membres
        </button>
      </div>

      {/* 2️⃣ SÉLECTION CANDIDATS */}
      <div className="bg-white/10 p-3 rounded-3xl shadow-lg text-white mb-6 w-full max-w-2xl">
              ...
              <input
                ...
                onChange={() => {
                  setSelectedCandidats(prev => {
                    let updated;
                    if (prev.includes(c.id)) updated = prev.filter(id => id !== c.id);
                    else updated = [...prev, c.id];
                    ...
                    setFormData(prevForm => ({ ...prevForm, hommes, femmes }));
      ...
      {/* 3️⃣ FORMULAIRE */}
      <div ref={formRef} className="bg-white/10 rounded-3xl p-6 shadow-lg mb-6 max-w-2xl w-full">
        <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-4">
          ...
          <div className="col-span-2 mt-4">
            ...
            {rapportSuccess && <p className="text-green-600 font-semibold text-center mt-4 animate-pulse">✅ Rapport ajouté !</p>}
          </div>
        </form>
      </div>

      {/* 4️⃣ FILTRES */}
      <div className="bg-white/10 p-6 rounded-2xl shadow-lg mb-6 flex justify-center gap-4 flex-wrap text-white max-w-2xl w-full">
      ...
      {/* 5️⃣ TABLEAU / CARTES MOBILE */}
      {showTable && (
        <div className="w-full max-w-5xl flex flex-col gap-4 mb-6">
          {groupedReports.map(([monthKey,monthRapports])=>{
            const [year,monthIndex]=monthKey.split("-").map(Number);
            const monthLabel=`${getMonthNameFR(monthIndex)} ${year}`;
            const totalMonth=monthRapports.reduce((acc,r)=>{acc.hommes+=Number(r.hommes||0); acc.femmes+=Number(r.femmes||0); return acc;},{hommes:0,femmes:0});
            const isExpanded=expandedMonths[monthKey]||false;

            return (
              <div key={monthKey} className="space-y-2">
                {/* Collapse par mois */}
                <div className="flex items-center justify-between bg-white/20 px-4 py-2 rounded-lg cursor-pointer border-l-4 border-blue-500" onClick={()=>toggleMonth(monthKey)}>
                  <span className="text-white font-semibold">{isExpanded?"➖ ":"➕ "}{monthLabel}</span>
                  <span className="text-orange-400 font-semibold">Total : {totalMonth.hommes+totalMonth.femmes}</span>
                </div>
                ...
                {(isExpanded||monthRapports.length===1) && monthRapports.map(r=>{
                  ...
                  return (
                    <div key={r.id} className="bg-white/10 p-4 rounded-xl text-white flex flex-col md:flex-row md:items-center md:justify-between hover:bg-white/20 transition">
                      {/* Desktop tableau */}
                      <div className="hidden md:flex w-full text-sm font-semibold">
                        <div className="min-w-[200px]">{formatDateFR(r.date)}</div>
                        <div className="min-w-[200px] text-center">{r.baptise_par}</div>
                      ...
                      {/* Mobile carte */}
                      <div className="flex flex-col md:hidden gap-1 text-sm">
                        <div>📅 {formatDateFR(r.date)}</div>
                        <div><strong>Baptisé par :</strong> {r.baptise_par}</div>
                        <div>Hommes : {r.hommes}</div>
                        <div>Femmes : {r.femmes}</div>
                        <div className="text-orange-400 font-semibold">Total : {total}</div>
                        <button onClick={()=>handleEdit(r)} className="text-orange-400 underline hover:text-orange-500 mt-1 w-max">Modifier</button>
                      </div>
                    </div>
                  );
                })}
          ...
          {/* TOTAL GLOBAL */}
          <div className="flex items-center px-4 py-3 mt-2 border-t border-white/50 bg-white/10 rounded-b-xl text-white font-bold justify-between">
            <span>Total général</span>
            <span>Hommes : {totalGlobal.hommes}</span>
            <span>Femmes : {totalGlobal.femmes}</span>
            <span className="text-orange-400 font-semibold">Total : {totalGlobal.hommes+totalGlobal.femmes}</span>
          </div>
        </div>
      )}
