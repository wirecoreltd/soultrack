...
export default function MembresCellule() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // -------- CONSEILLERS --------
        const { data: conseillersData } = await supabase.from("profiles").select("id, prenom, nom");
  ...
  const getCelluleNom = (celluleId) => {
    return cellules.find(c => c.id === celluleId)?.cellule_full || "—";
  };
    ...
    return (
      <div key={m.id} className="bg-white p-4 rounded-xl shadow-md border-l-4 relative hover:shadow-lg transition">
        ...
        <div className="mt-2 text-sm text-black space-y-1 text-left">
          ...
          <p>🏠 Cellule : {m.cellule_id ? getCelluleNom(m.cellule_id) : "—"}</p>
        ...
        {/* Détails étendus */}
        {isOpen && (
          <div className="text-black text-sm mt-2 w-full space-y-1 text-left">
        ...
        <tbody>
          {filteredMembres.map(m => (
            <tr key={m.id} className="border-t hover:bg-gray-50 transition">
              ...
              <td className="px-4 py-2">{m.cellule_id ? getCelluleNom(m.cellule_id) : "—"}</td>
  ...
  return (
    <div className="min-h-screen p-6" style={{ background: "linear-gradient(135deg,#2E3192,#92EFFD)" }}>
      ...
      {view === "card" ? (
        <>{filteredMembres.map(renderMemberCard)}</>
      ) : (
        renderTable()
      )}
