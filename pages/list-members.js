...
export default function ListMembers() {
  ...
  const handleStatusUpdateFromEnvoyer = (id, currentStatus) => {
    ...
    setPopupMember(null);
    setToastMessage("✅ Message envoyé et suivi enregistré !");
  ...
  return (
    ...
    >
      {/* TOAST MESSAGE */}
      {toastMessage && (
        <div className="absolute top-4 right-4 bg-green-500 text-white px-4 py-2 rounded shadow z-50 animate-fade-in">
          {toastMessage}
        </div>
      )}
      ...
      {view === "table" && (
        <div className="w-full max-w-6xl overflow-x-auto transition duration-200">
          <table className="w-full text-sm text-left border-separate border-spacing-y-1 table-auto">
            <thead className="text-white text-sm uppercase">
              <tr>
                <th className="px-4 py-2 w-1/3">Nom complet</th>
            ...
            <tbody>
              ...
              {nouveauxFiltres.length > 0 && (
                <tr>
                  <td colSpan={4} className="px-4 py-1 text-white font-semibold">
              ...
              {anciensFiltres.length > 0 && (
                <>
                  <tr>
                    <td colSpan={4} className="px-4 py-1 font-semibold text-white">
                      Membres existants
                    </td>
                  </tr>
