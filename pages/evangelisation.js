...
```
  ...
  <div className="flex flex-col sm:flex-row gap-2 mb-4 items-center w-full max-w-5xl">
    <select
      ...
      className="border rounded-xl px-4 py-2 text-gray-800 shadow-md w-full sm:w-1/2"
    >
      <option value="">📍 Choisir type destinataire</option>
    ...
    {selectedTarget && (
      <BoutonEnvoyerContacts
        ...
        cellule={selectedTargetType === "cellule" ? cellules.find(c => c.id === selectedTarget) : null}
        conseiller={selectedTargetType === "conseiller" ? conseillers.find(c => c.id === selectedTarget) : null}
        onEnvoye={(id) => {
