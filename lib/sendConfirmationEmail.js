...
// Envoie un email de confirmation avec facture HTML en pièce jointe
...
async function generateInvoiceHTML({ invoiceNumber, planNom, planPrix, dureeLabel, egliseNom, date }) {
  const html = `
    <!DOCTYPE html>
    <html>
    <head>
...
export async function sendConfirmationEmail({ egliseId, planId, planNom, planPrix, provider, duree = "1m" }) {
  try {
    ...
    const dureeLabel    = DUREE_LABELS[duree] || "1 mois";
    const providerLabel = PROVIDER_LABELS[provider] || provider;

    const htmlBase64 = await generateInvoiceHTML({
      invoiceNumber,
      planNom,
      planPrix,
      dureeLabel,
    ...
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: [eglise.email],
        subject: `✅ Confirmation — Plan ${planNom} activé (${dureeLabel})`,
        html: `
          <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:30px;">
            ...
            <p>
              Votre accès au plan <strong>${planNom}</strong> pour <strong>${dureeLabel}</strong>
              (${planPrix}) a bien été activé via <strong>${providerLabel}</strong>.
            </p>
