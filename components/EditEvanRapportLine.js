...
export default function EditEvanRapportLine({ isOpen, onClose, rapport, onSave }) {
  ...
  const fields = [
    ...
    { key: "priere", label: "Prière", type: "number" },
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-2xl p-6 w-80 shadow-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex justify-end gap-3 mt-4">
          <button onClick={onClose} className="px-4 py-2 bg-gray-400 rounded hover:bg-gray-500 text-white">Annuler</button>
          <button onClick={handleSave} className="px-4 py-2 bg-blue-500 rounded hover:bg-blue-600 text-white">Enregistrer</button>
        </div>
