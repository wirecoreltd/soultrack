...
Avec amour en Christ ❤️`;
  ...
  const inputClass = (hasError) =>
    `w-full p-2 text-black rounded ${hasError ? "border-2 border-red-500 outline-red-500" : ""}`;
  ...
  return (
    <div className="min-h-screen bg-[#333699] text-white p-4 flex flex-col items-center">
      ...
      <div ref={formRef} className="w-full max-w-md bg-white/10 p-6 rounded-xl space-y-4">
        ...
        {/* ✅ Titre "Envoyer à" */}
        <h2 className="text-lg font-semibold text-emerald-300 border-b border-white/20 pb-2">
          Envoyer à
        </h2>
        ...
        <button onClick={handleAction} className="w-full bg-blue-600 hover:bg-blue-700 py-2 rounded font-semibold transition-colors">
      ...
      <div className="w-full max-w-5xl overflow-x-auto">
        ...
        {/* Header desktop — 7 colonnes alignées avec les rows */}
        <div className="hidden md:grid md:grid-cols-[1.3fr_1fr_1fr_1fr_1fr_0.8fr_0.8fr] text-sm font-semibold uppercase border-b border-white/40 pb-2 gap-x-3 px-4">
        ...
        {invitations.map((inv) => {
          ...
          return (
            <div
              key={inv.id}
              className={`grid grid-cols-1 md:grid-cols-[1.3fr_1fr_1fr_1fr_1fr_0.8fr_0.8fr] gap-x-3 px-4 py-3 mt-3 items-center border-l-4 ${statusStyle.border} bg-white/5 rounded-lg`}
            >
              {/* Mobile : chaque champ avec son label */}
              <div>
              ...
              <div className="flex justify-start md:justify-center gap-2 text-sm mt-2 md:mt-0">
                {inv.statut === "pending" && (
                  <button
